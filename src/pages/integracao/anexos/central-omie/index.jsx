...
export const IntegracaoAnexosCentralOmieEsteira = () => {
  const [searchTerm, setSearchTerm] = useStateWithStorage(
    "esteira_integracao_anexos_central_omie_search_term"
  );

  const [time, setTime] = useStateWithStorage(
    "esteira_integracao_anexos_central_omie_time",
    1
  );

  const filters = {
    direcao: INTEGRACAO_DIRECAO_MAP.CENTRAL_OMIE,
    tipo: INTEGRACAO_TIPO_MAP.ANEXOS,
    time: time,
  };

  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["integracao-anexos-central-omie-listar", { filters }],
    ...
    searchTerm?.toLowerCase()?.trim()?.length > 2
      ? data?.results?.filter((ticket) => {
          ...
          return (
            ...
            ticket?.payload?.anexos?.documento
              ?.toLowerCase()
          ...
          <SelectTime
            ...
            onValueChange={(value) => {
              queryClient.invalidateQueries([
                "integracao-anexos-central-omie-listar",
              ]);
